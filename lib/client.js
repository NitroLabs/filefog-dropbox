...
Client.prototype.searchFiles = function(filters,options) {
    ...
    this._getClient().then(function (client) {
        // TODO: Convert client.deferred to the Q api
        client.delta(options.page_token, function(err,response){
            var fileMap = {};
            var content = {};
            // Handle fetch error
            if (err) {
            ...
            // Extract the files from the filemap
            content.items = Object.keys(fileMap).map(function(key) {
                return fileMap[key]._json;
            });

            // Add the nextPageToken if there are still more files
            if (response.shouldPullAgain){
                content.page_token = response.cursorTag
            }
