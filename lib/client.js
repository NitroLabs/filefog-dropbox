...
CClient.prototype.searchFiles = function(filters,options) {
    options = extend({}, options || {});
    filters = extend({}, filters || {});
    
    // XXX: Actually respect these parameters    
    if (options.maxResults){
        //params.file_limit = options.maxResults;
    }
    var path = "/";
    if (options.parent){
    ...
    return this._getClient().then(function (client) {
        ...
        function iterateFiles(cursorTag){
            client.delta(cursorTag,function(err,response){
                if (err) {
                    err = errorHandler(err);
                    return deferred.reject(err);
                }
                // Copy all files to the files object
                for (var i=0; i<response.changes.length; i++){
                    var file = response.changes[i];
                    if (file.wasRemoved){
                        delete fileMap[file.path];
                    } else {
                        fileMap[file.path]=file.stat;    
                    }
                }
                // Pull more files or return the result
                if (response.shouldPullAgain){
                    iterateFiles(response.cursorTag);
                } else {
        ...
        // Return the final promise object (for files)
        return iterateFiles().then(function(fileMap){
            var files = Object.keys(fileMap).map(function(key) {
                return fileMap[key]._json;
            });
            return files
        });
    });
