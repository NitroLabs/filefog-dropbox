...
transform.getFileInformation = function (file_response){
    ...
    transform.downloadUrl = file_response.path;
...
transform.searchFiles = function(items_response){
    var self = this;
    var transform = {};
    transform.content = items_response.map(function(current_item){
        new_item = {}
        new_item.mimetype = current_item.mimetype;
        new_item.download_url = current_item.path;
        new_item.is_file = !current_item.is_dir;
        new_item.is_folder = current_item.is_dir;
        new_item.identifier = current_item.path;
        new_item.modified_date = new Date(current_item.modified);
        new_item.revision = current_item.revision;
        new_item.name = current_item.path.split('/').pop();
        new_item.file_size = current_item.bytes;
        new_item._raw = current_item;
        return new_item;
    }).filter(function(current_item){
        // Remove folders
        return current_item.is_file;
    });
    transform.total_items = transform.items.length;
    transform.page_token = transform.page_token;
    return transform;
}
